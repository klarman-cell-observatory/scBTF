...
class FactorizationSet:
    ...
    def cophenetic_correlation_elbow_plot(self, ax=None, n_clusters: int = 4):
        ...
        coph_cor = pd.DataFrame({
            "Rank": self.get_ranks(),
            "Cophenetic Correlation": [self.cophenetic_correlation(self.consensus_matrix(rank, n_clusters=n_clusters)) for rank in self.get_ranks()]
        })
        if ax is None:
            _, ax = plt.subplots(figsize=(3, 3))
    ...
    def gene_cophenetic_correlation_elbow_plot(self, ax=None):
        ...
        coph_cor = pd.DataFrame({
            "Rank": self.get_ranks(),
            "Cophenetic Correlation": [self.cophenetic_correlation(self.gene_consensus_matrix(rank=rank)) for rank in self.get_ranks()]
        })
        if ax is None:
            _, ax = plt.subplots(figsize=(3, 3))
        sns.pointplot(coph_cor, x='Rank', y='Cophenetic Correlation', color='m', ax=ax)
    ...
    def plot_gene_across_components(self, rank: int, gene: str, restart_index: int):
        ...
        ax = sns.scatterplot(x=torch.arange(gene_factors.shape[1]), y=gene_factors[gene_index, :], s=40, color="k", ax=ax)
    ...
    def rank_metrics_plot(self):
        var_explained = pd.DataFrame({
            rank: [self.variance_explained(rank, i).item() for i in range(len(self.factorizations[rank].keys()))] for rank in self.get_ranks()
        })
        coph_cor = pd.DataFrame({"Rank": self.get_ranks(),
            "Cophenetic Correlation": [self.cophenetic_correlation(self.gene_consensus_matrix(rank=rank, entropy = -1e3, eps = -1e3)) for rank in self.get_ranks()]
        })
        sample_coph_cor = pd.DataFrame({"Rank": self.get_ranks(),
            "Cophenetic Correlation": [self.cophenetic_correlation(self.consensus_matrix(rank, n_clusters=min(rank, len(self.sc_tensor.sample_list)))) for rank in self.get_ranks()]
        })
        ...
        axs[0].errorbar(var_explained.mean(0).index, var_explained.mean(0).values, yerr=var_explained.std(0).values, fmt='.-', color='g')
    ...
    def reconstruct_factors_from_median_gene_factor(self, rank, n_clusters=None):
        ...
        medians = np.stack(np.median(data_normed[np.where(labels_ == group)[0], :], axis=0) for group in np.unique(labels_))
    ...
    def plot_components(self,
        ...
        for factor, program in gene_programs.items():
            ...
            if plot_erichment_terms:
                ...
                labeled_factors.loc[len(labeled_factors.index)] = ['gene set enrichment', '.', str(factor), 0, top_terms]

        g = sns.FacetGrid(labeled_factors, row='factor', col='mode', hue='type', height=1.5, aspect=2.5, sharex='col', sharey=False, palette='tab20')
    ...
    def get_labeled_sample_factor(self, rank: int, restart_index: int, grouping_column: str = 'sample_type', melt: bool = False):
        ...
        if melt:
            dH = dH.melt(id_vars=self.sc_tensor.sample_features.columns.tolist() + ['index'], var_name='factor', value_name='value')
        return dH
    ...
    def get_gene_programs(self,
        ...
        mm = (selected_features.T-selected_features.min(axis=1))/(selected_features.max(axis=1)-selected_features.min(axis=1))
        mm = mm.T
        ...
        res = [pd.DataFrame({'gene':gene_prog, 
                  ...
                  'absolute':[-selected_features.loc[g,f] for g in gene_prog]}) for f, gene_prog in gene_programs.items()]
        gene_programs = {i: list(res[i].sort_values(by=['relative', 'entropy','absolute']).gene.values) for i in range(len(gene_programs))}
        if sort_by == 'entropy':
    ...
    def gene_consensus_matrix(self, rank: int, normalize_gene_factors: bool = True, 
        ...
        for restart in restarts:
            gene_programs, argmx = self.get_gene_programs(rank, restart, normalize_gene_factors=normalize_gene_factors, 
                threshold=threshold, sort_by=sort_by, entropy=entropy, eps=eps, return_argmx=True)
    ...
    def gene_factor_elbow_plots(self, rank, restart_index, factor_index = None, ncols = 5, num_genes = 20, fontsize = 9, normalize=True):
        ...
        if factor_index is not None:
            ...
            [ax.text(i, load[i], load.index[i], rotation='vertical', verticalalignment='bottom', horizontalalignment='center', fontsize=fontsize) for i in range(num_genes)]
